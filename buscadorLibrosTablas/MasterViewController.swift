...
class MasterViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    func refresh(sender: AnyObject) {
        ...
        let entidadLibro = NSEntityDescription.entityForName("Event", inManagedObjectContext: self.contexto!)
        let peticion = entidadLibro?.managedObjectModel.fetchRequestTemplateForName("petLibros")
        if libros.count > 0 {
        
        do {
            let entidadLibros = try self.contexto?.executeFetchRequest(peticion!)
            for libro in entidadLibros! {
                ...
                libros.append(Libro(ISBN: ISBN, titulo: titulo, autores: autores, portada: portada))
